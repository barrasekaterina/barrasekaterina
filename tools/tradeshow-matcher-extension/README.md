...
Works identically to the Flask version - see its README's "Status, Tags,
Match Confidence and Matched By" section for the full design and
rationale. Short version: `Status` is exactly one of `Existing Contact`
(wins even over a Lead match), `Existing Lead`, `New Contact` (no
personal match, but the company already exists in CRM), or `New Lead`
(company not in CRM either) - nothing else mixed in. A separate `Tags`
column carries everything that used to stack onto the old combined
Status string (`Also a Lead`, `Duplicate`, `Bank/CU`, `Existing Domain`,
...
run, and merges back `NMLS FullName` (that individual's own name from
NMLS, to visually compare against the attendee's own Full Name), `NMLS
RegulationType`, `NMLS LicensingStatus`, `NMLS LocationNMLSID`, and `NMLS
LocationName` (the company/branch that person is currently authorized to
represent). Same backend, same Windows
